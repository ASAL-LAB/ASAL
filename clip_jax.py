...
class MyFlaxCLIPWrapper():
    """Advanced wrapper for CLIP model integration"""
    def __init__(self, model_name="clip-vit-base-patch32"):
        self.processor = AutoProcessor.from_pretrained(f"openai/{model_name}")
        self.model = FlaxCLIPModel.from_pretrained(f"openai/{model_name}")
        
        # Image normalization parameters
        self.image_normalizer = jnp.array(self.processor.image_processor.image_mean)
        self.image_scaler = jnp.array(self.processor.image_processor.image_std)
    ...
    def embed_text(self, prompts):
        ...
        z_text = self.model.get_text_features(input_ids=inputs['input_ids'], attention_mask=inputs['attention_mask'])
    ...
    def embed_img(self, img):
        ...
        returns shape (D)
        """
        img = rearrange((img-self.image_normalizer)/self.image_scaler, "H W C -> 1 C H W")
        z_img = self.model.get_image_features(img)[0]
